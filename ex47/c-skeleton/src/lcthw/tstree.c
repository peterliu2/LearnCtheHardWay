/**
 * @file tstree.c
 * @brief Ternary Search Tree implement
...
void *TSTree_search(TSTree *root, const char *key, size_t len)
{
	...
	if (node) {
		//assert(node->value != NULL && "return 'node->value' must not be NULL.");
		return node->value;
	} else {
		return NULL;
	}
...
void *TSTree_search_prefix(TSTree *root, const char *key, size_t len)
{
	...
	/* traverse until we find the first value in the equal chain, this is then the first node with this prefix */
