/**
 * @file bstree_tests.c
 * @brief Binary searching tree unit test
...
char *test_traverse(void)
{
	...
	mu_assert(rc == 0, "Failed to traverse on good.");
	mu_assert(traverse_called == 3, "Wrong traverse count on good.");

	traverse_called = 0;
	rc = BSTree_traverse(map, traverse_fail_cb);
	mu_assert(rc == 1, "Failed to traverse on fail.");
	mu_assert(traverse_called == 2, "Wrong traverse count on fail.");
...
{
	bstring delete = (bstring)BSTree_delete(map, &test1);
	mu_assert(delete != NULL, "Got NULL on delete.");
	mu_assert(delete == &expect1, "Should get expect1.");
	bstring result = BSTree_get(map, &test1);
	mu_assert(result == NULL, "Should delete.");
	
	delete = (bstring)BSTree_delete(map, &test1);
	mu_assert(delete == NULL, "Should got NULL on delete.");


	delete = (bstring)BSTree_delete(map, &test2);
	mu_assert(delete != NULL, "Got NULL on delete.");
	mu_assert(delete == &expect2, "Should get expect2.");
	result = BSTree_get(map, &test2);
	mu_assert(result == NULL, "Should delete.");
	
	delete = (bstring)BSTree_delete(map, &test3);
	mu_assert(delete != NULL, "Got NULL on delete.");
	mu_assert(delete == &expect3, "Should get expect3.");
	result = BSTree_get(map, &test3);
	mu_assert(result == NULL, "Should delete.");

	/* test deleting non-existent stuff */
	delete = (bstring)BSTree_delete(map, &test3);
	mu_assert(delete == NULL, "Should get NULL.");

	return NULL;
}

char *test_fuzzing(void) 
{
