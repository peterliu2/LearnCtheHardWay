/**
 * @file bstree.c
 * @brief Binary Search Tree implement
...
static inline void BSTree_set_node(BSTree *map, BSTreeNode *node, void *key, void *data)
{
	int cmp = map->compare(node->key, key);

	if (cmp <= 0) {
		if (node->left) {
			BSTree_set_node(map, node->left, key, data);
		} else {
			node->left = BSTreeNode_create(node, key, data);
		}
	} else {
		if (node->right) {
			BSTree_set_node(map, node->right, key, data);
		} else {
...
int BSTree_set(BSTree *map, void *key, void *data)
{
	if (map->root == NULL) {
		...
		check(map->root != NULL, "Failed to create root,");
	} else {
		BSTree_set_node(map, map->root, key, data);
	}
	return 0;
...
static inline BSTreeNode *BSTree_get_node(BSTree *map, BSTreeNode *node ,void *key)
{
	int cmp = map->compare(node->key, key);

	if (cmp < 0) {
		if (node->left) {
			return BSTree_get_node(map, node->left, key);
		} else {
			return NULL;
		}
	} else if (cmp > 0){
		if (node->right) {
			return BSTree_get_node(map, node->right, key);
		} else {
			return NULL;
		}
	} else {
...
{
	if (map->root) {
		BSTreeNode *node = BSTree_get_node(map, map->root, key);
...
static inline BSTreeNode *BSTree_node_delete(BSTree *map, BSTreeNode *node, void *key)
{
	...
	if (cmp < 0) {
		...
		if (node->left && node->right) {
			...
			/* this leaves the old successor with possiblely a right child
