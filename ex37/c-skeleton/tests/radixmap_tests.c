/**
 * @file radixmap_tests.c
 * @brief Radix sort unit test
...
static int check_order(RadixMap *map)
{
	...
	for (i = 0; map->end > 0 && i < map->end - 1; i++) {
		...
		if (d1.data.key > d2.data.key) {
			debug("FAIL: i=%u, key: %u, value: %u, equals max? %s\n", i, d1.data.key, d1.data.value, d2.data.key == UINT32_MAX ? "true":"false");
			return 0;
		}
	}
	return 1;
...
static char *test_operations(void)
{
	size_t N = 2000;
	...
	mu_assert(check_order(map), "RadixMap didn't stay sorted order after search test.");
