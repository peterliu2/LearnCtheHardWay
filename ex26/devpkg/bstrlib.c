...
reallocStrategy:
			...
			if (NULL ==
				(x = (unsigned char *)bstr__alloc((size_t) len))) {
...
{
	if (a == NULL || a->data == NULL || a->mlen < a->slen ||
		a->slen < 0 || a->mlen == 0 || NULL == s || len < 0
		|| len >= INT_MAX)
		return BSTR_ERR;
...
int biseqcaselessblk(const_bstring b, const void *blk, int len)
{
	...
	for (i = 0; i < len; i++) {
		if (b->data[i] != ((unsigned char *)blk)[i]) {
			...
			if (c !=
				(unsigned char)downcase(((unsigned char *)blk)[i]))
				return 0;
		}
	}
...
int biseqblk(const_bstring b, const void *blk, int len)
{
	if (len < 0 || b == NULL || blk == NULL || b->data == NULL
		|| b->slen < 0)
		return BSTR_ERR;
...
int breplace(bstring b1, int pos, int len, const_bstring b2, unsigned char fill)
{
	...
	if (aux->slen != len)
		bstr__memmove(b1->data + pos + aux->slen,
					  b1->data + pos + len, b1->slen - (pos + len));
...
static int findreplaceengine(bstring b, const_bstring find,
							 const_bstring repl, int pos, instr_fnptr instr)
{
	...
	if (BSTR_OK == (ret = balloc(b, b->slen + acc + 1))) {
		...
		for (i = slen - 1; i >= 0; i--) {
			...
			if (l) {
				bstr__memmove(b->data + s + acc, b->data + s,
							  l);
			}
